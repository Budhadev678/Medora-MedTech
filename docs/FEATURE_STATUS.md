...
| **2** | **App Shell & Role Dashboards (Prompt 1)** | `VERIFIED` | Role-aware layouts: Mobile-first `PatientShell` with Emergency SOS & More drawer; High-density `ProfessionalShell` with collapsible sidebar, `OrganizationSwitcher`, `NotificationPanel`, `UserMenu`, `PageHeader`, `Breadcrumbs`, and 79 compiled routes with standardized empty states for future phases. |
