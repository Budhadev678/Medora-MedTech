...
export default function AccessDeniedPage() {
  ...
  const name = activePersona?.name || "Guest";
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 max-w-lg mx-auto">
      ...
      <p className="text-xs text-slate-600 mb-6 leading-relaxed">
        You are authenticated as <strong className="text-slate-900">{name} ({userRole})</strong>. This area is restricted to authorized personnel in accordance with MEDORA security and clinical governance policies.
      </p>
      <div className="flex flex-wrap gap-3 justify-center">
        <Link href={ROLE_DASHBOARD_ROUTES[userRole]}>
          <Button size="sm" className="gap-2 text-xs">
            <ArrowLeft className="h-4 w-4" /> Go to {userRole.toUpperCase()} Dashboard
          </Button>
        </Link>
        <Link href="/login">
