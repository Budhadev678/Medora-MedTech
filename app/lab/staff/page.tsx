...
const SEED_LAB_STAFF: LabStaffMember[] = [
  {
    id: "LAB-STAFF-1001",
    fullName: "Dr. B. Mohapatra, MD (Pathology)",
    qualification: "MBBS, MD (Pathology), FICP",
    role: "Chief Pathologist & Quality Signatory",
    councilReg: "OMC-1998-1204",
    isSignatory: true,
    specialization: "Hematology & Histopathology",
    status: "ON_DUTY",
    currentStation: "Pathology Verification & Sign-off",
    phone: "+91 674 2550110",
    email: "b.mohapatra@abcdiagnostics.medora",
  },
  {
    id: "LAB-STAFF-1002",
    fullName: "Dr. Sunita Tripathy, PhD",
    qualification: "M.Sc, PhD (Medical Biochemistry)",
    role: "Senior Clinical Biochemist",
    councilReg: "AIIMS-BBSR-2015",
    isSignatory: true,
    specialization: "Clinical Chemistry & Immunoassays",
    status: "ON_DUTY",
    currentStation: "Automated Chemistry Analyzers (Cobas 6000)",
    phone: "+91 674 2550111",
    email: "sunita.t@abcdiagnostics.medora",
  },
  {
    id: "LAB-STAFF-1003",
    fullName: "Prakash Jena",
    qualification: "B.Sc Medical Laboratory Technology (BMLT)",
    role: "Senior Laboratory Technologist",
    councilReg: "NABL-LT-2018-091",
    isSignatory: false,
    specialization: "Specimen Accessioning & CBC Analyzers",
    status: "ON_DUTY",
    currentStation: "Intake Accessioning & Sysmex XN",
    phone: "+91 674 2550112",
    email: "prakash.j@abcdiagnostics.medora",
  },
  {
    id: "LAB-STAFF-1004",
    fullName: "Anjali Behera",
    qualification: "Certified Medical Phlebotomist (CMLT)",
    role: "Lead Phlebotomist",
    councilReg: "PHLEB-OD-2021-334",
    isSignatory: false,
    specialization: "Venipuncture & Cold-Chain Transport",
    status: "ON_DUTY",
    currentStation: "Outpatient Phlebotomy Collection #1",
    phone: "+91 674 2550113",
    email: "anjali.b@abcdiagnostics.medora",
  },
];
...
export default function LabStaffPage() {
  ...
  const filteredStaff = SEED_LAB_STAFF.filter((staff) => {
    ...
    if (searchQuery.trim()) {
      ...
      const matchReg = staff.councilReg.toLowerCase().includes(q);
      const matchRole = staff.role.toLowerCase().includes(q);
      const matchSpec = staff.specialization.toLowerCase().includes(q);
      return matchName || matchId || matchReg || matchRole || matchSpec;
    }
    return true;
  });
  ...
  return (
    <RoleGuard allowedRoles={["hospital_admin", "lab_staff", "staff", "admin", "doctor"]}>
      <div className="space-y-6 max-w-7xl mx-auto pb-24 font-sans p-4 sm:p-6 animate-in fade-in-50 duration-200">
        <PageHeader
          title="Laboratory Personnel & Diagnostic Specialists"
          description="Certified pathologists, medical biochemists, laboratory technologists, and licensed phlebotomists."
          breadcrumbs={[{ label: "Diagnostic Lab", href: "/lab" }, { label: "Staff Directory" }]}
        />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="bg-white rounded-2xl border-slate-200 shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Laboratory Staff</p>
                <h3 className="text-2xl font-black text-slate-900 font-mono mt-0.5">{SEED_LAB_STAFF.length}</h3>
                <p className="text-[10px] text-teal-600 font-semibold mt-0.5">NABL Certified Roster</p>
              </div>
          ...
          <Card className="bg-white rounded-2xl border-slate-200 shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Active On Duty</p>
                <h3 className="text-2xl font-black text-emerald-900 font-mono mt-0.5">
                  {SEED_LAB_STAFF.filter((s) => s.status === "ON_DUTY").length}
                </h3>
                <p className="text-[10px] text-emerald-600 font-semibold mt-0.5">Phlebotomy & Analyzers</p>
              </div>
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-indigo-900 font-mono mt-0.5">
                  {SEED_LAB_STAFF.filter((s) => s.isSignatory).length}
                </h3>
                <p className="text-[10px] text-indigo-600 font-semibold mt-0.5">Pathologist Digital Sign</p>
              </div>
          ...
          <Card className="bg-white rounded-2xl border-slate-200 shadow-xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-base font-black text-slate-900 font-mono mt-1">OMC Registered</h3>
                <p className="text-[10px] text-teal-600 font-semibold mt-0.5">100% Medical Council Verified</p>
              </div>
              <div className="h-10 w-10 rounded-xl bg-teal-50 border border-teal-200 text-teal-700 flex items-center justify-center">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-1.5 p-1 bg-slate-100 rounded-xl">
            ...
            >
              All Specialists ({SEED_LAB_STAFF.length})
            </Button>
            <Button
            ...
            >
              On Duty ({SEED_LAB_STAFF.filter((s) => s.status === "ON_DUTY").length})
            </Button>
            <Button
            ...
            >
              Signatories ({SEED_LAB_STAFF.filter((s) => s.isSignatory).length})
            </Button>
          </div>
          ...
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <Input
              placeholder="Search Specialist Name, Reg, or Specialization..."
              value={searchQuery}
              ...
              <CardContent className="p-5 pt-0 space-y-3 text-xs">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <strong className="font-mono text-slate-800">{staff.councilReg}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Specialization:</span>
                    <span className="font-semibold text-teal-800">{staff.specialization}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Current Station:</span>
                    <span className="text-slate-700">{staff.currentStation}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">NABL Electronic Sign:</span>
                    {staff.isSignatory ? (
                      <span className="text-emerald-700 font-bold flex items-center gap-1">
                        <ShieldCheck className="h-3 w-3" /> Certified Signatory
                      </span>
                    ) : (
                      <span className="text-slate-400">Technical Operator</span>
                    )}
                  </div>
                </div>
